...
const currentScenarioArt = drawBoard(
    game.width,
    game.height,
    game.scenario.player,
    game.scenario.enemies,
    game.scenario.food
)
...
function printStatus() {
    ...
    process.stdout.write(
        `Average time per work unit: ${Controller.workCount /
            Controller.totalWorkTime}ms\n`
    )
    ...
    process.stdout.write(
        `Outcome right now is ${JSON.stringify(game.scenario.outcome)}\n`
    )
