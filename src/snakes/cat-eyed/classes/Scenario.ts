...
class Scenario {
    ...
    public get moveOdds(): { move: Move; odds: number }[] {
        return Object.keys(this.outcomeByMove)
            .map((move: Move) => ({
                move,
                odds:
                    this.outcomeByMove[move].lose /
                    (this.outcomeByMove[move].lose +
                        this.outcomeByMove[move].win +
                        this.outcomeByMove[move].unknown)
            }))
            .sort((a, b) => a.odds - b.odds)
    }

    public get bestMove(): Move {
        return this.moveOdds[0] ? this.moveOdds[0].move : 'up'
    }
