...
class Scenario {
  ...
  public get outcomeByMove(): Partial<OutcomeByMove> {
    ...
    const possibilitiesByMove = this.children.reduce((acc, child) => {
        return {
        ...acc,
    ...
    const outcomes = this.children.reduce((acc: Partial<OutcomeByMove>, child: Scenario) => {
    ...
    Object.keys(outcomes)
        ...
        outcomes[move].lose = outcomes[move].lose / possibilitiesByMove[move],
        outcomes[move].unknown = outcomes[move].unknown / possibilitiesByMove[move]
        })
  ...
  public get safestMove(): Move {
    return this.moveOdds.sort((a, b) => a.loseOdds - b.loseOdds)[0]?.move || 'up'
  }
