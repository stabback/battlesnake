...
class Board {
    ...
    constructor(data: BoardData, youId: string) {
        ...
        this.food = data.food
        this.snakes = data.snakes.map(
            snakeData => new Snake(snakeData, this.width, this.height)
        )
    ...
    isPointSafe(point: Point, youId: string): boolean {
        ...
        const smallerSnakes = this.enemySnakes.filter(
            snake => snake.body.length < you.body.length
        )
        if (smallerSnakes.some(snake => snake.intersects(point, true, false)))
            return false

        const threatSnakes = this.enemySnakes.filter(
            snake => snake.body.length >= you.body.length
        )
        if (threatSnakes.some(snake => snake.intersects(point, true, true)))
            return false
