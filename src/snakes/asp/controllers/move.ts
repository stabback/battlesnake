...
function move(
    ...
    if (!resolvedMove) {
        ...
        logger.log(game, {
            message: `[Move] Biggest enemy is ${biggestEnemy.name} ${
                biggestEnemy.body.length
            }`,
            points: [
                {
                    ...biggestEnemy.head,
        ...
        if (game.player.body.length >= biggestEnemy.body.length + 2) {
            ...
            logger.log(
                game,
                '[Move] Attack strategy resolved to ' + resolvedMove
            )
        } else {
    ...
    if (!resolvedMove) {
        ...
        logger.log(
            game,
            `[Move] Eat strategy returned ${resolvedMove || 'null'}`
        )
    }
