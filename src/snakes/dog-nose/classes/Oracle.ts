...
class OracleClass {
    ...
    public doWork() {
        ...
        if (this.workQueue.length === 0 || start - this.runtime > MAX_RUNTIME) {
            this.workDone()
            return
        }
    ...
    public updateBaseScenario(scenario: Scenario) {
        ...
        const existingScenario = game.scenario.children
            ? game.scenario.children.find(child => child.id === scenario.id)
            : null
