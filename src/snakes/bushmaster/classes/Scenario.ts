...
class Scenario {
    ...
    public get outcome(): number {
        ...
        const childOutcome =
            this.children.reduce((acc, child) => {
                console.assert(
                    !isNaN(child.outcome),
                    'Outcome for child is not a number',
                    this,
                    child
                )
    ...
    private calculateScore() {
        ...
        this.rubric.playerHealth =
            this.profile.playerHealth.impact * this.player.health

        this.rubric.enemyHealth =
            this.enemies.reduce((acc, enemy) => acc + enemy.health, 0) *
            this.profile.enemyHealth.impact
        ...
        if (biggestEnemy) {
            const relativeImpact =
                (this.player.body.length - biggestEnemy.body.length) *
                this.profile.relativeLength.impact
            this.rubric.relativeLength = Math.min(
                Math.max(relativeImpact, -20),
                20
            )
        }

        this.rubric.enemyCount =
            this.enemies.length * this.profile.enemyCount.impact
        ...
        this.enemies.forEach(snake => {
            ...
            if (snake.body.length < this.player.body.length) {
                this.rubric.smallSnakeProximity =
                    this.profile.smallSnakeProximity.impact / proximity
            } else {
                this.rubric.dangerSnakeProximity =
                    this.profile.dangerSnakeProximity.impact / proximity
            }
        })
        ...
        this.score = Object.values(this.rubric).reduce(
            (sum, item) => sum + item
        )
    }
    ...
    public createChildren() {
        ...
        const possibleMoves = this.snakes.map(snake =>
            snake.possibleNextHeadPositions.map(point => ({
                ...point,
                id: snake.id
            }))
        )

        const possibleCombinations = fastCartesian(possibleMoves) as {
            x: number
            y: number
            id: string
        }[][]
        ...
        this.children = possibleCombinations.map(combination => {
            ...
            const newSnakes: Snake[] = combination.map(newLocation => {
                ...
                const didEat = this.history.ate.some(
            ...
            const survivedSnakes = newSnakes.filter(snake => {
                ...
                const isInBounds = isPointOnBoard(
                    snake.head,
                    this.height,
                    this.width
                )
