...
function move(
    ...
    const enemies = game.board.snakes
        .filter(snake => snake.id !== player.id)
        .map(snake => new Snake(snake, game.board.width, game.board.height))
    ...
    if (rootScenario.children.length === 0) {
        ...
        resolvedMove = getMoveFromPoints(
            game.you.head,
            rootScenario.children[0].player.head
        )
            ...
            const sortedMoves = Object.entries(moves)
                ...
                .map(([m, outcomes]: [Move, number[]]) => {
                    const averageOutcome =
                        outcomes.reduce((acc, outcome) => outcome + acc, 0) /
                        outcomes.length
