...
function drawBoard(
    ...
    enemies.forEach((enemy, index) =>
        healthBar.push(
            ENEMY_STYLES[index].color(
                `${ENEMY_STYLES[index].headToken}: ${enemy.health}`
            )
        )
    )
    ...
    // Fill in enemies
    enemies.forEach((snake, index) =>
        drawSnake(
            grid,
            snake,
            ENEMY_STYLES[index].headToken,
            ENEMY_STYLES[index].color
        )
    )
    ...
    try {
        ...
        snake.body.forEach(segment => {
            if (prevSegment) {
                ...
                board[segment.y][segment.x] = colorFunc(
                    DIRECTION_TOKEN_MAP_SYMBOL[direction]
                )
            }
        ...
        throw new Error(
            'Attempting to draw a snake which has parts that are not on the board!'
        )
    }
}
